...
import {
  CreateCardRequest,
  CreateCardResponse, DeleteCardRequest, DeleteCardResponse,
  FindCardsRequest, FindCardsResponse,
  ReadCardRequest,
...
export class CardsService {
  ...
  async create(input: CreateCardRequest, customerId: string): Promise<CreateCardResponse> {
    try {
      ...
      return {
        card: cardToProtoCard(cardSource),
      };
    } catch (error) {
  ...
  async read(input: ReadCardRequest, customerId: string): Promise<CreateCardResponse> {
    try {
      ...
      const card = await this.stripeClient.customers.retrieveCard(customerId, input.id);

      return {
        card: cardToProtoCard(card),
      };
    } catch (error) {
      this.logger.log(error);
      throw new RpcException(error);
  ...
  async delete(input: DeleteCardRequest, customerId: string): Promise<DeleteCardResponse> {
    try {
      ...
      const [customer, card] = await Promise.all([
        ...
        this.stripeClient.customers.retrieveCard(customerId, input.id),
      ]);
  ...
  async list(input: FindCardsRequest, customerId: string): Promise<FindCardsResponse> {
    try {
      ...
      const cardObjs = await this.stripeClient.customers.listSources(customerId, {object: 'card'});
      const cardsList = cardObjs.data as unknown as Stripe.cards.ICard[];

      return {
        cards: cardSliceToProtoCardSlice(cardsList),
      };
    } catch (error) {
      this.logger.log(error);
      throw new RpcException(error);
