...
import { ReadSubscriptionResponse, TenantSubscription } from '@ultimatebackend/proto-schema/billing';
...
export class GetSubscriptionHandler implements IQueryHandler<GetSubscriptionQuery> {
  ...
  async execute(query: GetSubscriptionQuery): Promise<ReadSubscriptionResponse> {
    this.logger.log(`Async ${query.constructor.name}...`);

    try {
      const { input } = query;

      if (!input.id) { throw new RpcException('Missing subscription id input'); }
