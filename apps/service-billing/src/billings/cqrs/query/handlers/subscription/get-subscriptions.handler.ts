...
import { FindSubscriptionsResponse, TenantSubscription } from '@ultimatebackend/proto-schema/billing';
...
export class GetSubscriptionsHandler implements IQueryHandler<GetSubscriptionsQuery> {
  ...
  async execute(query: GetSubscriptionsQuery): Promise<FindSubscriptionsResponse> {
    this.logger.log(`Async ${query.constructor.name}...`);
    const { input, identity } = query;
    ...
    try {
      ...
      if (result.data && result.data.length <= 0) {  return {subscriptions: []}; }
