...
import { Invoice, ReadInvoiceResponse } from '@ultimatebackend/proto-schema/billing';
...
export class GetInvoiceHandler implements IQueryHandler<GetInvoiceQuery> {
  ...
  async execute(query: GetInvoiceQuery): Promise<ReadInvoiceResponse> {
    ...
    if (!id) { throw new RpcException('Missing Invoice id input'); }

    try {
      // Check cache to see if data exist
      const cacheData = await this.cacheStore.get<Invoice>('service-billing/invoice/' + id);
      ...
      await this.cacheStore.set('service-billing/invoice/' + id, invoice, {ttl: 100});
      return {
        invoice,
      };
    } catch (e) {
