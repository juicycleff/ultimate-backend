...
import { ReadTenantResponse, Tenant } from '@ultimatebackend/proto-schema/tenant';
...
export class GetTenantHandler implements IQueryHandler<GetTenantQuery> {
  ...
  async execute(query: GetTenantQuery): Promise<ReadTenantResponse> {
    ...
    try {
      if (!where) { throw new RpcException('Missing where inputs'); }
      ...
      const tenant =  await this.tenantRepository.findOne({...filter, ...userFilter });
      return {
