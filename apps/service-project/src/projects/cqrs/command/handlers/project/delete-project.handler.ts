...
import { DeleteProjectResponse, Project } from '@ultimatebackend/proto-schema/project';
...
export class DeleteProjectHandler implements ICommandHandler<DeleteProjectCommand> {
  ...
  async execute(command: DeleteProjectCommand): Promise<DeleteProjectResponse> {
    ...
    try {
      ...
      const project = await this.projectRepository.findOne({normalizedName: input.id}, true);
      if (!project) {
