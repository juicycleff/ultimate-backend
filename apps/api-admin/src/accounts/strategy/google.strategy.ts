...
export class GoogleStrategy extends PassportStrategy(OAuth2Strategy) {
  ...
  constructor(
    ...
    super({
      clientID: config.get<string>('app.auth.google.clientID'),
      clientSecret: config.get<string>('app.auth.google.clientSecret'),
      callbackURL: config.get<string>('app.auth.google.callbackURL'),
      scope: ['email', 'profile'],
    });
  }
