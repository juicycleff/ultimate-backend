...
export class MultiTenantService implements MongoOptionsFactory {
  ...
  async createMongoOptions(): Promise<MongoModuleOptions> {
    ...
    const appConString = this.generateMongoConnectionString(
      this.tenantServiceConfig.dbUri, tenantInfo.tenantId, this.tenantServiceConfig.dbReplicaOptions);

    const tenantName = tenantInfo.tenantId;
    let databseName;
    /**
