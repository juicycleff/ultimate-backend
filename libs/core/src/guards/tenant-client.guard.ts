...
@Injectable()
export class TenantClientGuard implements CanActivate {
  constructor(
  ...
  async canActivate(
    ...
    try {
      const tenant = (await this.httpService.get(
        (process.env.TENANT_ENDPOINT_REST || (`http://localhost:${AppConfig.services?.tenant.port || '9200'}`)) + '/tenant/find-client', {
        data: {
