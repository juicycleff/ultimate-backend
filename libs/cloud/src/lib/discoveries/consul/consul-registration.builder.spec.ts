...
describe('ConsulRegistrationBuilder', () => {
  let client = new ConsulRegistrationBuilder('http://localhost', 9000);
  ...
  it('generates host dynamically', async () => {
    client = new ConsulRegistrationBuilder('http://localhost', 9000);
    const t = client
      .host(null)
      .instanceId('2')
      .serviceName('service-name')
  ...
  it('throw error if port is missing', async () => {
    client = new ConsulRegistrationBuilder('http://localhost', null);
    const t = () => {
      client
        .serviceName('service-name')
        .discoveryOptions({
